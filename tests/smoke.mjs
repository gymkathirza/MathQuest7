...
assert.ok(indexSource.includes('realmCompanion'),'Home/practice must include companion strip mount');
assert.ok(indexSource.includes('awardCoinsBtn')&&indexSource.includes('data-award-coins'),'Parent dashboard must expose award-coins UI');
...
const clear=awardDayClearRewards(rewardState,'ns_signs');
assert.equal(clear.coins,COINS_DAY_CLEAR);
assert.equal(rewardState.dayClearCoinClaimed.ns_signs,true);
const br=awardBreakBonus(rewardState);
assert.equal(br.coins,COINS_BREAK_BONUS);
// One-time backfill: coins only for cleared days not yet marked claimed; idempotent; live clears not reclaimable.
const backfill={coins:10,xp:100,cleared:{a:true,b:true,c:false},dayClearCoinClaimed:{}};
assert.deepEqual(eligibleDayClearCoinIds(backfill).sort(),['a','b']);
const backfillPreview=dayClearCoinBackfillPreview(backfill);
assert.equal(backfillPreview.days,2);
assert.equal(backfillPreview.coins,2*COINS_DAY_CLEAR);
const claimed=claimDayClearCoinBackfill(backfill);
assert.equal(claimed.ok,true);
assert.equal(claimed.coins,200);
assert.equal(backfill.coins,210);
assert.equal(backfill.xp,100,'Backfill grants coins only, not XP');
assert.equal(backfill.dayClearCoinClaimed.a,true);
assert.equal(backfill.dayClearCoinClaimed.b,true);
const again=claimDayClearCoinBackfill(backfill);
assert.equal(again.ok,false);
assert.equal(again.coins,0);
assert.equal(backfill.coins,210,'Second claim must not double-pay');
awardDayClearRewards(backfill,'c');
backfill.cleared.c=true;
assert.equal(dayClearCoinBackfillPreview(backfill).days,0,'Live day-clear award must mark day so backfill cannot reclaim');
...
assert.equal(strip.isPreview,false);
const stripPreview=companionStripView({realm:[],pets:[],activePet:null},{type:'pet',id:'otter'});
assert.equal(stripPreview.visible,true);
assert.equal(stripPreview.petGhost,true);
assert.equal(stripPreview.pet.id,'otter');
assert.ok(stripPreview.previewLabel.includes('Probability Otter'));
const stripGhostB=companionStripView({realm:['signpost'],pets:[],activePet:null},{type:'building',id:'bridge'});
assert.equal(stripGhostB.ghostBuilding.id,'bridge');
assert.ok(stripGhostB.isPreview);
const parentAward={coins:25};
const awarded=awardParentCoins(parentAward,100);
assert.equal(awarded.ok,true);
assert.equal(awarded.coins,100);
assert.equal(parentAward.coins,125);
assert.equal(awardParentCoins(parentAward,0).ok,false);
assert.equal(awardParentCoins(parentAward,-5).ok,false);
assert.equal(awardParentCoins(parentAward,10001).ok,false);
assert.equal(parentAward.coins,125,'Rejected awards must not change coins');
assert.ok(!/fetch\s*\(|XMLHttpRequest|navigator\.sendBeacon/.test(readFileSync(join(ROOT,'js','rewards.mjs'),'utf8')),'Rewards helpers must stay local-only (no network)');
...
console.log('PASS: 20 topics; benchmarks; mastery replay; open-ended focus; learner insights; GIF boost; mastery day log; coins/realm/pets/preview; day-clear coin backfill; active practice timer');
