...
function uniqueChoices(answer,candidates=[]){
  const correct=String(answer), set=new Set([correct]);
  for(const candidate of candidates){if(String(candidate)!==correct)set.add(String(candidate));if(set.size===4)break;}
  let guard=0;
  while(set.size<4&&guard++<100){
    if(!Number.isNaN(Number(answer))){const d=R(-10,10)||1;set.add(String(Number(answer)+d));}
    else set.add(`choice-${guard}`);
  }
  return [...set].slice(0,4).sort(()=>Math.random()-.5);
}
function problem(q,a,explain,level,extra={}){return{q,a:String(a),choices:uniqueChoices(a),explain,level,...extra}}
function fractionProblem(q,numerator,denominator,explain,level,extra={}){
  const ans=`${numerator}/${denominator}`;
  const candidates=[`${Math.max(1,numerator-1)}/${denominator}`,`${numerator+1}/${denominator}`,`${numerator}/${denominator+1}`,`${Math.max(1,denominator-numerator)}/${denominator}`,`1/${denominator}`];
  return {q,a:ans,choices:uniqueChoices(ans,candidates),explain,level,...extra};
}
...
function complexGeometryStats(topicId){
 ...
 if(topicId==='sp_prob'){const red=R(2,7),blue=R(2,7),green=R(1,5),total=red+blue+green;return fractionProblem(`A bag has ${red} red, ${blue} blue, and ${green} green gems. What is P(red or blue)?`,red+blue,total,`Favorable outcomes=${red+blue}; total=${total}; probability=${red+blue}/${total}.`,'complex',{strategy:'Favorable ÷ total'})}
...
export function generateComplexProblem(topicId){if(topicId.startsWith('ns_'))return complexInteger(topicId);if(topicId.startsWith('rp_'))return complexRatio(topicId);if(topicId.startsWith('ee_'))return complexAlgebra(topicId);return complexGeometryStats(topicId)}
...
export function generateNCWordProblem(topicId){
 ...
 if(topicId==='sp_prob'){const red=R(2,8),blue=R(2,8),total=red+blue;return fractionProblem(`At a fictional ${loc} math fair, a prize bag has ${red} red and ${blue} blue tokens. What is P(red)?`,red,total,`There are ${red} favorable outcomes out of ${total} total.`,'word',{location:loc,strategy:'Favorable ÷ total'})}
...
export function generateTieredProblem(topicId,level){if(level==='standard')return {...generateProblem(topicId),level:'standard'};if(level==='complex')return generateComplexProblem(topicId);if(level==='word')return generateNCWordProblem(topicId);throw new Error(`Unknown practice level: ${level}`)}
export function generateDailyBenchmark(topicId){const questions=[];for(let i=0;i<LEVEL_COUNTS.standard;i++)questions.push(generateTieredProblem(topicId,'standard'));for(let i=0;i<LEVEL_COUNTS.complex;i++)questions.push(generateTieredProblem(topicId,'complex'));for(let i=0;i<LEVEL_COUNTS.word;i++)questions.push(generateTieredProblem(topicId,'word'));return questions.map((q,i)=>({...q,number:i+1}))}
