...
- `js/` — ES modules / app logic (`app.js`, `curriculum.mjs`, `daily-session.mjs`, `practice-timer.mjs`, `mastery-session.mjs`, `learner-insights.mjs`, `coach-visuals.mjs`, `rewards.mjs`). Sibling imports use `./name.mjs`; anything the module needs from the web root (e.g. `version.json`, `sw.js`) is resolved with `new URL('../file', import.meta.url)`.
