...
- Version: `0.14.0`
- Release label: `mastery-replay-open-ended`
...
- `css/` — `app.css`.
- `js/` — `app.js`, `curriculum.mjs`, `daily-session.mjs`, `practice-timer.mjs`, `mastery-session.mjs` (modules reference web-root files via `new URL('../file', import.meta.url)`).
...
- Open-Ended Mastery Quest (beyond Day 20): unlocks only after all 20 days are cleared; shows a full NC.7 / Union County domain recap and endless mixed advanced practice spanning all 20 topics
